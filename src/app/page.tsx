...
import { Company as TCompany, Job as TJob, CompanyCategory } from "@/prisma/client";
import { getJob, getCompany, getCategory } from "@/app/_home/action/fetch";

export default async function Home() {
  const jobs: TJob[] = await getJob();
  const companies: TCompany[] = await getCompany();
  const categories: CompanyCategory[] = await getCategory();
  ...
  return (
    <div className="flex flex-col min-h-[calc(100vh-6rem)]">
      ...
      <div className="flex flex-col gap-3 sm:gap-4 w-[98%] sm:w-[95%] max-w-5xl mx-auto my-3 md:my-4 relative -top-20 sm:-top-14 md:-top-32">
        <JobList data={jobs} />
        <CompanyList data={companies} categories={categories} />
      </div>
    </div>
  );
}